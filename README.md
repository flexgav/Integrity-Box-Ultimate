...
* 🔑 **Центр Keybox:** облачный каталог Keybox, кэширование, локальный импорт XML из `/sdcard/Download` и `/sdcard/Documents`, выбор активного Keybox и проверка его состояния в **Проверке целостности**.
...
* 🎯 **Target Box:** автоматическая сборка `target.txt` для Tricky Store, защита ручного режима, импорт собственного списка и резервная копия перед изменениями.
* 🧪 **Target Simulator:** точечная настройка аттестации для конкретных приложений, если отдельный банк, Wallet или сервис требует другой режим проверки.
...
* 🧹 **Спуф ROM Props:** автоматическая и ручная очистка следов кастомных прошивок: LineageOS, crDroid, Evolution X, DerpFest, RisingOS, AOSPA, PixelExperience, Pixel, Infinity, Havoc, Axion и Generic.
...
* 🧰 **GMS Tools:** глубокая очистка GMS, сброс Google Wallet, очистка данных выбранных приложений и повторная подготовка устройства к проверке Play Маркета.
* 🤖 **AutoPilot:** фоновое обслуживание Keybox, PIF, Target Box и патчей безопасности по расписанию без ручной рутины.
...
2. 📲 **Прошейте IntegrityBox Ultimate:** Установите zip-архив модуля. Во время установки модуль автоматически синхронизирует каталог Keybox из облака, готовит совместимость с metamodule-решениями и останавливает старые фоновые процессы предыдущей версии.
...
5. ▶️ **Запустите основное действие:** Перейдите в список модулей Root-менеджера и нажмите **Action** на карточке IntegrityBox Ultimate. Дождитесь завершения: модуль обновит Keybox, проверит расписание PIF/Target и применит только те этапы, которые действительно пора обновлять.
...
7. 🧼 **Глубокая очистка GMS:** Перейдите в **Инструментарий** → **GMS Tools** и выполните **Глубокая очистка данных**. Это удалит старые состояния сертификации Google и данные Google Services Framework. Следуйте подсказке, чтобы **перезагрузить** устройство. *(Вы выйдете из аккаунта Google.)*
...
10. 🧹 **Спуф ROM Props при необходимости:** Если устройство работает на кастомной прошивке, откройте **Спуфинг кастомных ROM** → **Спуф ROM Props** и нажмите **Авторежим** — модуль сам определит, следы каких ROM-семейств есть на устройстве, и включит нужные карточки очистки.
...
> [!IMPORTANT]
> Не нажимайте **Action** несколько раз подряд. В v42.0 критичные операции защищены от дублирующего запуска, но лучше дождаться завершения текущего цикла: повторный запуск может быть пропущен как уже активная операция.
...
5. 🧰 **Антидетект-флаги:** В **Разное** → **Настройки модуля** включайте только нужные пункты: **Debug Fingerprint**, **Debug Build**, **Build Tag**, **Очистка LSposed**, **Подмена шифрования**, **Скрыть Recovery**, **Очистка логов Gapps** и **Архив логов менеджера**. Для очистки следов прошивки используйте **Спуфинг кастомных ROM** → **Спуф ROM Props**.
...
4. Для точечной настройки отдельных приложений откройте **Настройка Tricky Store** -> **Target Simulator**: там можно выбрать приложение, профиль и режим, а также импортировать собственный target-файл.
...
> [!NOTE]
> Начинайте с простых действий: проверка Keybox, **Глубокая очистка данных**, перезагрузка и повторная проверка Play Маркета. Target Simulator, Boot Hash Spoofer и Nuke используйте только когда понятно, какая именно проверка не проходит.
...
3. Если Keybox отсутствует или статус не `ONLINE`, откройте WebUI -> **Центр Keybox** -> **Keybox Loader**. Сверху выберите **Слот 1** *(основной слот)*, нажмите **Пересобрать каталог**, дождитесь обновления списка, затем нажмите на свежий Keybox с бейджем `ПРОВАЙДЕР` и подтвердите загрузку в **Слот 1**. Если cloud-список не появился, проверьте интернет и доступ к GitHub. Если синхронизация все равно недоступна, положите свой XML Keybox в `/sdcard/Download` или `/sdcard/Documents`, нажмите кнопку обновления списка и загрузите найденный `ЛОКАЛЬНЫЙ` Keybox в **Слот 1**.
4. После обновления Keybox выполните WebUI -> **Инструментарий** -> **GMS Tools** -> **Глубокая очистка данных**, перезагрузите устройство и снова проверьте Play Маркет.
...
6. Если verdict целостности проходит, но Play Маркет все еще пишет **Устройство не сертифицировано**, откройте WebUI -> **Инструментарий** -> **GMS Tools** -> **Глубокая очистка данных**.
...
3. Если **Strong integrity** не проходит, вернитесь к сценарию **Play Integrity не проходит**: проверьте **Active Keybox**, обновите Keybox через **Keybox Loader**, затем выполните **Глубокая очистка данных** и перезагрузитесь.
4. Если **Strong integrity** проходит, но Wallet все равно не работает, откройте WebUI -> **Инструментарий** -> **GMS Tools** -> **Сброс Wallet**. Это очищает локальное состояние Wallet, которое могло запомнить прошлую ошибку безопасности.
...
2. Откройте WebUI -> **Настройка Tricky Store** -> **Target Simulator**.
