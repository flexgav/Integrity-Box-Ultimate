...
7. 🧼 **Глубокая очистка GMS:** Перейдите в **Play Integrity Fix** → **GMS Tools** и выполните **Глубокая очистка данных**. Это удалит старые состояния сертификации Google и данные Google Services Framework. Следуйте подсказке, чтобы **перезагрузить** устройство. *(Вы выйдете из аккаунта Google.)*
...
4. После обновления Keybox выполните WebUI -> **Play Integrity Fix** -> **GMS Tools** -> **Глубокая очистка данных**, перезагрузите устройство и снова проверьте Play Маркет.
...
6. Если verdict целостности проходит, но Play Маркет все еще пишет **Устройство не сертифицировано**, откройте WebUI -> **Play Integrity Fix** -> **GMS Tools** -> **Глубокая очистка данных**.
...
4. Если **Strong integrity** проходит, но Wallet все равно не работает, откройте WebUI -> **Play Integrity Fix** -> **GMS Tools** -> **Сброс Wallet**. Это очищает локальное состояние Wallet, которое могло запомнить прошлую ошибку безопасности.
